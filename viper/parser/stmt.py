...
class Stmt(object):
    ...
    def call(self):
        ...
        if isinstance(self.stmt.func, ast.Name) and self.stmt.func.id in stmt_dispatch_table:
            ...
            contract_address = parse_value_expr(self.stmt.func.value.args[0], self.context)
            ...
            contract_address = parse_value_expr(ast.parse('self.token_address').body[0].value, self.context)
            return external_contract_call_stmt(self.stmt, self.context, contract_name, contract_address)

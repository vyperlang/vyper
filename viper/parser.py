...
from .exceptions import CompilationContext, VariableDeclarationException, \
    StructureException, InvalidLiteralException, ConstancyViolationException, \
    InvalidTypeException, TypeMismatchException, ParserException
...
def get_defs_and_globals(code):
    ...
    for item in code:
        try:
            ...
            e.set_err_pos(item.lineno, item.col_offset)
            raise
...
def get_func_details(code):
    ...
    for arg in code.args.args:
        try:
            ...
            e.set_err_pos(arg.lineno, arg.col_offset)
            raise
...
class Context():
    ...
    def new_variable(self, name, typ, item):
        try:
    ...
    def new_placeholder(self, typ, item):
        name = '_placeholder_'+str(self.placeholder_count)
        self.placeholder_count += 1
        return self.new_variable(name, typ, item)
...
def parse_func(code, _globals, origcode, _vars=None):
    # TODO add try catch ParserException
    name, args, output_type, const, sig, method_id = get_func_details(code)
    for arg in args:
    ...
    try:
        ...
        # Byte array literals
            ...
            for c in expr.s:
                ...
                bytez += bytes([ord(c)])
            placeholder = context.new_placeholder(ByteArrayType(len(bytez)), expr)
            seq = []
        ...
        # Function calls
            ...
            # Slice, eg. slice("mongoose", start=3, len=5) -> "goose"
                ...
                if len(expr.args) != 1:
                    raise StructureException("Expecting only one non-keyword argument: the bytearray", expr.args[1])
                ...
                placeholder_node = LLLnode.from_list(context.new_placeholder(sub.typ, expr.func), typ=sub.typ, location='memory')
            ...
            # Byte array concatenation, eg. concat("Mon", "goose") -> "Mongoose"
                ...
                # Node representing the position of the output in memory
                placeholder = context.new_placeholder(ByteArrayType(total_maxlen), expr.func)
            ...
            # SHA3 hashing
                ...
                if len(expr.args) != 1:
                    raise StructureException("Expecting only one non-keyword argument: the input", expr.args[1])
                sub = parse_expr(expr.args[0], context)
                ...
                if not isinstance(sub.typ, ByteArrayType):
                    raise TypeMismatchException("SHA3 argument must be byte array", expr.args[0])
                ...
                if sub.location == "calldata":
                    ...
                    raise Exception("Unsupported location: %s" % sub.location)
                placeholder = context.new_placeholder(sub.typ, expr.args[0])
        ...
        e.set_err_pos(expr.lineno, expr.col_offset)
        raise
...
def parse_stmt(stmt, context):
    try:
        if isinstance(stmt, ast.Expr):
            ...
            pos = context.new_variable(varname, typ, stmt)
            return LLLnode.from_list('pass', typ=None)
            ...
            if isinstance(stmt.targets[0], ast.Name) and stmt.targets[0].id not in context.vars:
                pos = context.new_variable(stmt.targets[0].id, set_default_units(sub.typ), stmt.targets[0])
        ...
        # Calls
            ...
            if stmt.func.id == 'send':
                ...
                if not is_base_type(to.typ, "address"):
                    raise TypeMismatchException("Expected an address as destination for send", stmt.args[0])
                value = parse_value_expr(stmt.args[1], context)
                if not is_base_type(value.typ, ("num", "num256")):
                    raise TypeMismatchException("Send value must be a positive integer!", stmt.args[1])
                else:
                ...
                if not is_base_type(sub.typ, "address"):
                    raise TypeMismatchException("%s expects an address!" % stmt.func.id, stmt.args[0])
        ...
        # for i in range(n): ... (note: n must be a nonzero positive constant integer)
            ...
            if len(stmt.iter.args) == 1:
                if not isinstance(stmt.iter.args[0], ast.Num):
                    raise StructureException("Repeat must have a nonzero positive integral number of rounds", stmt.iter.args[0])
                ...
                if isinstance(stmt.iter.args[0], ast.Num) and isinstance(stmt.iter.args[1], ast.Num):
                    ...
                    if not isinstance(stmt.iter.args[1].right, ast.Num):
                        raise StructureException("Repeat must have a nonzero positive integral number of rounds", stmt.iter.args[1].right)
                    start = parse_value_expr(stmt.iter.args[0], context)
                    rounds = stmt.iter.args[1].right.n
            varname = stmt.target.id
            pos = context.vars[varname][0] if varname in context.forvars else context.new_variable(varname, BaseType('num'), stmt.target)
        ...
        # Creating a new memory variable and assigning it
            ...
            if not isinstance(stmt.op, (ast.Add, ast.Sub, ast.Mult, ast.Div, ast.Mod)):
                raise Exception("Unsupported operator for augassign", stmt.op)
        ...
        # Return statement
            ...
            if context.return_type is None:
                if stmt.value:
                    raise TypeMismatchException("Not expecting to return a value", stmt.value)
        ...
        e.set_err_pos(stmt.lineno, stmt.col_offset)
        raise

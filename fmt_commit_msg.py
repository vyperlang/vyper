...
def format_commit_message(text, width=72):
    ...
    while i < len(lines):
        ...
        if is_list_item(line):
            ...
            wrapped = fill(content, width=width - len(prefix), 
        ...
        paragraph = []
        while i < len(lines) and lines[i].strip() and not is_list_item(lines[i]):
...
def main():
    ...
    parser.add_argument('file', nargs='?', default='commitmsg.txt',
                        help='File to format (default: commitmsg.txt)')

...
class Variable:
    ...
    def _introspect(self):
        if not self.node.simple:
            raise StructureException("Invalid structure for variable declaration", self.node)
        ...
        if self.node.value is None:
            ...
            if node.enclosing_scope == "global" and not self.is_constant:
                raise

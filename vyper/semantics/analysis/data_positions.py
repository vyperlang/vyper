...
def set_data_positions(
    ...
    if storage_layout_overrides is not None:
        ...
        # check that the generated layout file is the same as the input.
        # note this is reachable by user error, e.g. an override file with
        # extra entries or wrong `type`/`n_slots` fields.
        ...
        if roundtrip != storage_layout_overrides:
            ...
            hint = "did you provide a complete storage layout file, "
            hint += "as generated by `vyper -f layout`?"
            raise StorageLayoutException(msg, hint=hint)
    else:
...
def _fetch_path(path: list[str], layout: StorageLayout, node: vy_ast.VyperNode):
    ...
    for segment in path:
        if not isinstance(tmp, dict) or segment not in tmp:
            raise StorageLayoutException(
    ...
    if not isinstance(tmp, dict) or "slot" not in tmp:
...
def _allocate_with_overrides(vyper_module: vy_ast.Module, layout: StorageLayout):
    ...
    if GLOBAL_NONREENTRANT_KEY in layout:
        nonreentrant_slot = _fetch_path([GLOBAL_NONREENTRANT_KEY], layout, vyper_module)

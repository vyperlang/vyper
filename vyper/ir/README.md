...
IR is Vyper's current intermediate representation. It is used as "a high level assembly". While IR could theoretically compile to any backend which shares enough intrinsics with the EVM, this document assumes the reader has a good understanding of the EVM's execution model.

# Structure of IR

The grammar of IR is `(s_expr)` where `s_expr` is one of
...
jump to a label. Vyper’s dialect of IR has two forms, simple goto and goto with args. The latter form is useful for implementing subroutines.

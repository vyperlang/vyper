...
class VariableRangeAnalysis(IRAnalysis):
    ...
    def _narrow_nonzero(self, state: RangeState, var: IRVariable) -> None:
        """Narrow a variable's range to exclude zero (for assert conditions).

        For ranges spanning zero (lo < 0 < hi), we cannot represent
        [lo, -1] ∪ [1, hi] with a single contiguous range, so we
        conservatively don't narrow in that case.
        """
        ...
        if current.lo < 0:
            if current.hi < 0:
                # All negative values are non-zero, nothing to narrow
                return
            if current.hi == 0:
            ...
            # else: range spans zero [lo, hi] where lo < 0 < hi

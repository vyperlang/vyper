...
class MemoryCopyElisionPass(IRPass):
    ...
    def _process_basic_block(self, bb: IRBasicBlock):
        ...
        for i, inst in enumerate(bb.instructions.copy()):
            if inst.opcode == "mload":
                ...
                if src_loc.is_fixed and dst_loc.is_fixed:
                    ...
                    if src_loc.offset in mcopy_chain:
                        ...
                        if prev_inst.opcode in ("calldatacopy", "codecopy", "returndatacopy", "dloadbytes"):
                            # Can merge if sizes match and no hazards
                            if (prev_src_loc.size == src_loc.size == dst_loc.size and 
                                self._can_merge_special_copy_chain(bb, prev_inst, inst, src_loc, dst_loc)):
                                ...
                                self.updater.nop(prev_inst, annotation="[memory copy elision - merged " + prev_inst.opcode + "]")
                                # Update chain tracking
                                del mcopy_chain[src_loc.offset]
                                # Track the new special copy in the chain for potential future merging
                            ...
                            if (prev_src_loc.size == src_loc.size == dst_loc.size and 
                                self._can_merge_mcopy_chain(bb, prev_inst, inst, prev_src_loc, src_loc, dst_loc)):
                                ...
                                self.updater.update(inst, "mcopy", [size_op, IRLiteral(prev_src_loc.offset), dst_op])
                                # Remove the intermediate mcopy
                                self.updater.nop(prev_inst, annotation="[memory copy elision - merged mcopy]")
                                # Update chain tracking
                                del mcopy_chain[src_loc.offset]
                                mcopy_chain[dst_loc.offset] = (inst, prev_src_loc)
                                continue
            ...
            elif inst.opcode in ("call", "invoke", "create", "create2", "delegatecall", "staticcall"):
        ...
        if inst.opcode == "mstore":
            ...
            if isinstance(dst, IRLiteral):
                ...
                for dst_offset, (_, src_loc) in mcopy_chain.items():
                    ...
                    if (MemoryLocation.may_overlap(write_loc, src_loc) or
                        MemoryLocation.may_overlap(write_loc, dst_loc)):

from vyper.venom.analysis import DFGAnalysis, MemSSA, LivenessAnalysis
...
class DeadStoreElimination(IRPass):
    ...
    def _is_dead_store(self, mem_def) -> bool:
        ...
        for use_bb, uses in self.mem_ssa.memory_uses.items():
            for use in uses:
        ...
        if bb in self.mem_ssa.memory_defs:
            ...
            for next_def in defs:
                ...
                store_addr = store_inst.operands[1] if store_inst.opcode == "mstore" else store_inst.operands[0]
                next_addr = next_def.store_inst.operands[1] if next_def.store_inst.opcode == "mstore" else next_def.store_inst.operands[0]
                if (isinstance(store_addr, IRLiteral) and isinstance(next_addr, IRLiteral) and 
                    store_addr.value == next_addr.value):
                    return True

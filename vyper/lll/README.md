...
Vyper's current implementation of LLL can be referenced, mainly in [](../old_codegen/lll_node.py) (which describes the internal API for constructing LLL) and in [](../lll/compile_lll.py) (which describes how LLL is compiled to assembly and then to bytecode). Vyper's LLL output can be inspected by compiling a vyper contract with `vyper -f ir <contract.vy>`. Vyper also comes with a tool, `vyper-lll` which can be used to compile LLL directly to assembly or EVM bytecode.

...
def generate_lll_for_external_function(code, sig, context, check_nonpayable):
    ...
    if sig.is_init_func:
        ...
        for assign in code.get_children(vy_ast.Assign):
            typ = assign.target._metadata["type"]
            if not typ.is_immutable or assign.target.id in done:
                continue
            exit.append(
                [
                    "mstore",
                    ["add", "~codelen", typ.position.offset],
                    ["mload", context.globals[assign.target.id].pos],
                ]
            )

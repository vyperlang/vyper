...
def test_no_sources():
    with pytest.raises(KeyError):
        get_compilation_targets({})


def test_contracts_urls():
    with pytest.raises(JSONError):
        get_compilation_targets({"sources": {"foo.vy": {"urls": ["https://foo.code.com/"]}}})


def test_contracts_no_content_key():
    with pytest.raises(JSONError):
        get_compilation_targets({"sources": {"foo.vy": FOO_CODE}})
...
def test_contracts_return_value():
    ...
    assert result == {Path("foo.vy"): FOO_CODE, Path("contracts/bar.vy"): BAR_CODE}

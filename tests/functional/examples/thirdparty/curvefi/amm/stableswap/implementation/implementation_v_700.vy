...
def __init__(
    ...
    self.pool_contains_rebasing_tokens = 2 in self.asset_types
    __n_coins: uint256 = len(_coins)
    self.N_COINS = __n_coins
    self.N_COINS_128 = convert(__n_coins, int128)
    ...
    for i: int128 in range(self.N_COINS_128, bound=MAX_COINS_128):

        if i < self.N_COINS_128 - 1:
    ...
    self.call_amount = _call_amount
    self.scale_factor = _scale_factor
    self.rate_oracles = _rate_oracles
...
def _stored_rates() -> DynArray[uint256, MAX_COINS]:
    ...
    for i: int128 in range(self.N_COINS_128, bound=MAX_COINS_128):
        ...
        if self.asset_types[i] == 1 and not self.rate_oracles[i] == 0:

            # NOTE: fetched_rate is assumed to be 10**18 precision
            oracle_response: Bytes[32] = raw_call(
                convert(self.rate_oracles[i] % 2**160, address),
                abi_encode(self.rate_oracles[i] & ORACLE_BIT_MASK),
                max_outsize=32,
                is_static_call=True,
            )
        ...
        elif self.asset_types[i] == 3:  # IERC4626

            # fetched_rate: uint256 = IERC4626(coins[i]).convertToAssets(call_amount[i]) * scale_factor[i]
            # here: call_amount has IERC4626 precision, but the returned value is scaled up to 18
            # using scale_factor which is (18 - n) if underlying asset has n decimals.
            rates[i] = unsafe_div(
                rates[i] * staticcall IERC4626(self.coins[i]).convertToAssets(self.call_amount[i]) * self.scale_factor[i],
                PRECISION
            )  # 1e18 precision
...
def _balances() -> DynArray[uint256, MAX_COINS]:
    ...
    for i: int128 in range(self.N_COINS_128, bound=MAX_COINS_128):
        ...
        if self.pool_contains_rebasing_tokens:
            ...
            balances_i = staticcall IERC20(self.coins[i]).balanceOf(self) - self.admin_balances[i]
        else:
            # Use cached balances
...
def add_liquidity(
    ...
    if total_supply > 0:
        ...
        ys: uint256 = unsafe_div(D0 + D1, self.N_COINS)
        xs: uint256 = 0
        _dynamic_fee_i: uint256 = 0

        # Only account for fees if we are not the first to deposit
        base_fee: uint256 = unsafe_div(
            unsafe_mul(self.fee, self.N_COINS),
            unsafe_mul(4, unsafe_sub(self.N_COINS, 1))
        )
...
def remove_liquidity(
    ...
    assert len(_min_amounts) == self.N_COINS  # dev: invalid array length for _min_amounts
...
def get_y(
    ...
    y_prev: uint256 = 0
    c: uint256 = D
    Ann: uint256 = amp * self.N_COINS

    for _i: int128 in range(MAX_COINS_128):

        if _i == self.N_COINS_128:
            break
    ...
    c = c * D * A_PRECISION // (Ann * self.N_COINS)
...
def get_D(_xp: DynArray[uint256, MAX_COINS], _amp: uint256) -> uint256:
    ...
    for i: uint256 in range(255):
        ...
        for x: uint256 in _xp:
            D_P = D_P * D // x
        D_P //= pow_mod256(self.N_COINS, self.N_COINS)
        Dprev: uint256 = D

        # (Ann * S // A_PRECISION + D_P * N_COINS) * D // ((Ann - A_PRECISION) * D // A_PRECISION + (N_COINS + 1) * D_P)
        D = (
            (unsafe_div(Ann * S, A_PRECISION) + D_P * self.N_COINS) * D
            //
            (
                unsafe_div((Ann - A_PRECISION) * D, A_PRECISION) +
                unsafe_add(self.N_COINS, 1) * D_P
            )
        )
...
def _calc_withdraw_one_coin(
    ...
    ys: uint256 = unsafe_div((D0 + D1), unsafe_mul(2, self.N_COINS))
    ...
    for j: int128 in range(MAX_COINS_128):

        if j == self.N_COINS_128:
            break
...
def _get_p(
    ...
    ANN: uint256 = unsafe_mul(amp, self.N_COINS)
    Dr: uint256 = unsafe_div(D, pow_mod256(self.N_COINS, self.N_COINS))
...
def calc_token_amount(
    ...
    """
    return staticcall StableSwapViews(staticcall self.factory.views_implementation()).calc_token_amount(_amounts, _is_deposit, self)

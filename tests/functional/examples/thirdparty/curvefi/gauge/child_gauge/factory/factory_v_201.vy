...
def __init__(_root_factory: address, _root_impl: address, _crv: address):
    ...
    log UpdateRoot(_factory=_root_factory, _implementation=_root_impl)
...
def _psuedo_mint(_gauge: address, _user: address):
    ...
    if to_mint != 0 and self.crv != empty(IERC20):
        assert extcall self.crv.transfer(_user, to_mint, default_return_value=True)
...
def deploy_gauge(_lp_token: address, _salt: bytes32, _manager: address = msg.sender) -> address:
    ...
    salt: bytes32 = keccak256(abi_encode(chain.id, _salt))
...
def set_crv(_crv: IERC20):
    ...
    assert _crv != empty(IERC20)
    assert self.crv == empty(IERC20), "CRV already set"
...
def set_root(_factory: address, _implementation: address):
    ...
    log UpdateRoot(_factory=_factory, _implementation=_implementation)
...
def set_manager(_new_manager: address):
    ...
    log UpdateManager(_manager=_new_manager)

...
def initialize(_lp_token: address, _root: address, _manager: address):
    ...
    log SetGaugeManager(_gauge_manager=_manager)
...
def permit(
    ...
    if _owner.is_contract:
        ...
        assert convert(staticcall ERC1271(_owner).isValidSignature(digest, sig), bytes32) == ERC1271_MAGIC_VAL  # dev: invalid signature
    else:
...
def add_reward(_reward_token: address, _distributor: address):
    ...
    log AddReward(reward=_reward_token, index=reward_count)
    log SetDistributor(reward=_reward_token, distributor=_distributor)

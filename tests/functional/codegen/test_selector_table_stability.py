...
def test_dense_jumptable_stability_legacy():
    # test that the selector table data is stable across different runs
    # (xdist should provide different PYTHONHASHSEEDs).
    assert _expected_legacy_dense_jumptable_asm() in _compile_dense_jumptable(legacy_codegen=True)


def test_dense_jumptable_stability_venom():
    # Venom uses the same dense bucket ordering but emits entry_<selector> labels
    # instead of legacy function labels.
    assert _expected_venom_dense_jumptable_asm() in _compile_dense_jumptable(legacy_codegen=False)

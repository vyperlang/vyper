...
def test_counter_with_init(get_contract):
    ...
    code = (model
        .function("__init__(initial_count: uint256)")
        .deploy()
        .body(f"""
            {count} = initial_count
            {owner} = msg.sender
...
def test_flags_and_enums(get_contract):
    ...
    code = (model
        .function("create_order(amount: uint256, price: uint256) -> uint256")
        .external()
        .body(f"""
...
def test_nonreentrant_guards(get_contract):
    ...
    code = (model
        .function("protected_withdraw(amount: uint256)")
        .external()
        .nonreentrant()
        .body(f"""
            assert {bal} >= amount
...
def test_parametrized_with_constants(get_contract, decimals, multiplier):
    ...
    code = (model
        .function("deposit(tokens: uint256)")
        .external()

...
@pytest.mark.parametrize("typ", integer_types)
@pytest.mark.parametrize("literal", [-2, -1, 1, 2])
@pytest.mark.parametrize("op", ["add", "sub", "mul", "div"])
@pytest.mark.fuzzing
def test_unsafe_op_literal(get_contract, typ, literal, op):
    code = f"""
@external
def foo(x: {typ}) -> {typ}:
    ...
    int_info = parse_integer_typeinfo(typ)
    if not (not int_info.is_signed and literal < 0):
        c = get_contract(code)
        ...
        if int_info.is_signed:
            xs += [lo, lo + 1, -1, 0, 1, hi - 1, hi]
            for x in xs:
                expected = unsigned_to_signed(fn(x, literal) % mod_bound, int_info.bits)

                assert c.foo(x) == expected
        else:
            # 0x80 has some weird properties, like
            # it's a fixed point of multiplication by 0xFF
            fixed_pt = 2 ** (int_info.bits - 1)
            xs += [0, 1, hi - 1, hi, fixed_pt]
            for x in xs:
                assert c.foo(x) == fn(x, literal) % mod_bound

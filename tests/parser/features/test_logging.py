...
def test_log_dynamic_static_combo3(get_logs, get_contract, w3):
    code = """
...
def test_func(value: uint256,input: bytes[2048]):
    ...
    data2: bytes[2064] = "tytyty" # = concat(self.to_bytes(value),self.to_bytes(value),input)

    # log will be malformed
    log.TestLog(convert(value, bytes32),input,self.to_bytes(value))

    loggedValue: bytes[8] = self.to_bytes(value)
    # log will be normal
    log.TestLog(convert(value, bytes32),input,loggedValue)
    """
    ...
    assert logs[0].args.testData2 == b'x' * 132

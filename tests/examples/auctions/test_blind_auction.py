...
def test_late_bid(w3, auction_contract, w3_assert_tx_failed):
...
def test_too_many_bids(w3, auction_contract, w3_assert_tx_failed):
    k1 = w3.eth.accounts[1]
...
def test_early_reval(w3, auction_contract, w3_assert_tx_failed):
    k1 = w3.eth.accounts[1]
...
def test_late_reveal(w3, auction_contract, w3_assert_tx_failed):
    k1 = w3.eth.accounts[1]
...
def test_early_end(w3, auction_contract, w3_assert_tx_failed):
    k0 = w3.eth.accounts[0]

    # Should not be able to end auction before reveal time has ended
    w3_assert_tx_failed(lambda: auction_contract.auctionEnd(transact={"value": 0, "from": k0}))


def test_double_end(w3, auction_contract, w3_assert_tx_failed):
    k0 = w3.eth.accounts[0]

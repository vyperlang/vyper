...
@pytest.fixture
def auction_contract(w3, w3_get_contract, auction_start):
    with open("examples/auctions/simple_open_auction.vy") as f:
        contract_code = f.read()
        contract = w3_get_contract(contract_code, *[w3.eth.accounts[0], auction_start, EXPIRY])
    return contract
...
def test_bid(w3, tester, auction_contract, w3_assert_tx_failed):
    k1, k2, k3, k4, k5 = w3.eth.accounts[:5]
    # Bidder cannot bid 0
    w3_assert_tx_failed(lambda: auction_contract.bid(transact={"value": 0, "from": k1}))
    # Bidder can bid
    ...
    w3_assert_tx_failed(lambda: auction_contract.bid(transact={"value": 1, "from": k1}))
...
def test_end_auction(w3, tester, auction_contract, w3_assert_tx_failed):
    k1, k2, k3, k4, k5 = w3.eth.accounts[:5]
    # Fails if auction end time has not been reached
    w3_assert_tx_failed(lambda: auction_contract.endAuction())
    ...
    w3_assert_tx_failed(lambda: auction_contract.bid(transact={"value": 10, "from": k1}))

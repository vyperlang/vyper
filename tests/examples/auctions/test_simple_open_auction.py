...
def test_bid(w3, tester, auction_contract, assert_w3_tx_failed):
    k1, k2, k3, k4, k5 = w3.eth.accounts[:5]
    # Bidder cannot bid 0
    assert_w3_tx_failed(lambda: auction_contract.bid(transact={"value": 0, "from": k1}))
    # Bidder can bid
    ...
    assert_w3_tx_failed(lambda: auction_contract.bid(transact={"value": 1, "from": k1}))
...
def test_end_auction(w3, tester, auction_contract, assert_w3_tx_failed):
    k1, k2, k3, k4, k5 = w3.eth.accounts[:5]
    # Fails if auction end time has not been reached
    assert_w3_tx_failed(lambda: auction_contract.endAuction())
    ...
    assert_w3_tx_failed(lambda: auction_contract.bid(transact={"value": 10, "from": k1}))

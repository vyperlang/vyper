...
def test_initial_state(w3, w3_assert_tx_failed, w3_get_contract, get_balance, contract_code):
    # Inital deposit has to be divisible by two
    w3_assert_tx_failed(lambda: w3_get_contract(contract_code, value=13))
...
def test_abort(w3, w3_assert_tx_failed, get_balance, w3_get_contract, contract_code):
    ...
    w3_assert_tx_failed(lambda: c.abort(transact={"from": a2}))
    ...
    w3_assert_tx_failed(lambda: c.abort(transact={"from": a0}))


def test_purchase(w3, w3_get_contract, w3_assert_tx_failed, get_balance, contract_code):
    a0, a1, a2, a3 = w3.eth.accounts[:4]
    init_bal_a0, init_bal_a1 = get_balance()
    c = w3_get_contract(contract_code, value=2)
    # Purchase for too low/high price
    w3_assert_tx_failed(lambda: c.purchase(transact={"value": 1, "from": a1}))
    w3_assert_tx_failed(lambda: c.purchase(transact={"value": 3, "from": a1}))
    ...
    w3_assert_tx_failed(lambda: c.purchase(transact={"value": 2, "from": a3}))


def test_received(w3, w3_get_contract, w3_assert_tx_failed, get_balance, contract_code):
    a0, a1 = w3.eth.accounts[:2]
    init_bal_a0, init_bal_a1 = get_balance()
    c = w3_get_contract(contract_code, value=2)
    # Can only be called after purchase
    w3_assert_tx_failed(lambda: c.received(transact={"from": a1}))
    # Purchase completed
    c.purchase(transact={"value": 2, "from": a1})
    # Check that e.g. sender cannot trigger received
    w3_assert_tx_failed(lambda: c.received(transact={"from": a0}))
...
def test_received_reentrancy(w3, w3_get_contract, w3_assert_tx_failed, get_balance, contract_code):

...
def test_mint_and_burn(c, w3, assert_w3_tx_failed):
    ...
    assert c.balanceOf(a1) == 0
    assert_w3_tx_failed(lambda: c.burn(2, transact={"from": a1}))
    assert c.balanceOf(a1) == 0
    # Test scenario were mintes 0 to a2, burns (check balance consistency, false burn)
    c.mint(a2, 0, transact={"from": minter})
    assert c.balanceOf(a2) == 0
    assert_w3_tx_failed(lambda: c.burn(2, transact={"from": a2}))
    # Check that a1 cannot burn after depleting their balance
    assert_w3_tx_failed(lambda: c.burn(1, transact={"from": a1}))
    # Check that a1, a2 cannot mint
    assert_w3_tx_failed(lambda: c.mint(a1, 1, transact={"from": a1}))
    assert_w3_tx_failed(lambda: c.mint(a2, 1, transact={"from": a2}))
    # Check that mint to ZERO_ADDRESS failed
    assert_w3_tx_failed(lambda: c.mint(ZERO_ADDRESS, 1, transact={"from": a1}))
    assert_w3_tx_failed(lambda: c.mint(ZERO_ADDRESS, 1, transact={"from": minter}))


def test_totalSupply(c, w3, assert_w3_tx_failed):
...
def test_transfer(c, w3, assert_w3_tx_failed):
    minter, a1, a2 = w3.eth.accounts[0:3]
    assert_w3_tx_failed(lambda: c.burn(1, transact={"from": a2}))
    c.mint(a1, 2, transact={"from": minter})
    c.burn(1, transact={"from": a1})
    ...
    assert_w3_tx_failed(lambda: c.transfer(a1, 1, transact={"from": a2}))
    # Ensure 0-transfer always succeeds
    c.transfer(a1, 0, transact={"from": a2})


def test_maxInts(c, w3, assert_w3_tx_failed):
    minter, a1, a2 = w3.eth.accounts[0:3]
    c.mint(a1, MAX_UINT256, transact={"from": minter})
    assert c.balanceOf(a1) == MAX_UINT256
    assert_w3_tx_failed(lambda: c.mint(a1, 1, transact={"from": a1}))
    assert_w3_tx_failed(lambda: c.mint(a1, MAX_UINT256, transact={"from": a1}))
    # Check that totalSupply cannot overflow, even when mint to other account
    assert_w3_tx_failed(lambda: c.mint(a2, 1, transact={"from": minter}))
...
def test_transferFrom_and_Allowance(c, w3, assert_w3_tx_failed):
    ...
    assert_w3_tx_failed(lambda: c.transferFrom(a1, a3, 1, transact={"from": a2}))
    ...
    assert_w3_tx_failed(lambda: c.transferFrom(a1, a3, 1, transact={"from": a3}))
    ...
    assert_w3_tx_failed(lambda: c.transferFrom(a2, a3, 1, transact={"from": a1}))
...
def test_burnFrom_and_Allowance(c, w3, assert_w3_tx_failed):
    minter, a1, a2, a3 = w3.eth.accounts[0:4]
    assert_w3_tx_failed(lambda: c.burn(1, transact={"from": a2}))
    c.mint(a1, 1, transact={"from": minter})
    c.mint(a2, 1, transact={"from": minter})
    c.burn(1, transact={"from": a1})
    # This should fail; no allowance or balance (0 always succeeds)
    assert_w3_tx_failed(lambda: c.burnFrom(a1, 1, transact={"from": a2}))
    ...
    assert c.allowance(a2, a1) == 0
    # transferFrom should succeed when allowed, fail with wrong sender
    assert_w3_tx_failed(lambda: c.burnFrom(a2, 1, transact={"from": a3}))
    assert c.balanceOf(a2) == 1
    c.approve(a1, 1, transact={"from": a2})
    ...
    c.approve(a1, 1, transact={"from": a2})
    assert c.allowance(a2, a1) == 1
    assert_w3_tx_failed(lambda: c.burnFrom(a2, 1, transact={"from": a1}))
    # 0-approve should not change balance or allow transferFrom to change balance
    c.mint(a2, 1, transact={"from": minter})
    assert c.allowance(a2, a1) == 1
    c.approve(a1, 0, transact={"from": a2})
    ...
    assert_w3_tx_failed(lambda: c.burnFrom(ZERO_ADDRESS, 0, transact={"from": a1}))
...
def test_bad_transfer(c_bad, w3, assert_w3_tx_failed):
...
def test_bad_burn(c_bad, w3, assert_w3_tx_failed):
    ...
    assert_w3_tx_failed(lambda: c_bad.burn(3, transact={"from": a1}))


def test_bad_transferFrom(c_bad, w3, assert_w3_tx_failed):
    ...
    assert_w3_tx_failed(lambda: c_bad.transferFrom(a2, a1, 1, transact={"from": a1}))

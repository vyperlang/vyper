...
@pytest.fixture
def c(w3_get_contract, w3):
    with open("examples/tokens/ERC20.vy") as f:
        code = f.read()
    c = w3_get_contract(code, *[TOKEN_NAME, TOKEN_SYMBOL, TOKEN_DECIMALS, TOKEN_INITIAL_SUPPLY])
    return c


@pytest.fixture
def c_bad(w3_get_contract, w3):
    ...
    c = w3_get_contract(bad_code, *[TOKEN_NAME, TOKEN_SYMBOL, TOKEN_DECIMALS, TOKEN_INITIAL_SUPPLY])
    return c
...
def test_mint_and_burn(c, w3, w3_assert_tx_failed):
    ...
    assert c.balanceOf(a1) == 0
    w3_assert_tx_failed(lambda: c.burn(2, transact={"from": a1}))
    assert c.balanceOf(a1) == 0
    # Test scenario were mintes 0 to a2, burns (check balance consistency, false burn)
    c.mint(a2, 0, transact={"from": minter})
    assert c.balanceOf(a2) == 0
    w3_assert_tx_failed(lambda: c.burn(2, transact={"from": a2}))
    # Check that a1 cannot burn after depleting their balance
    w3_assert_tx_failed(lambda: c.burn(1, transact={"from": a1}))
    # Check that a1, a2 cannot mint
    w3_assert_tx_failed(lambda: c.mint(a1, 1, transact={"from": a1}))
    w3_assert_tx_failed(lambda: c.mint(a2, 1, transact={"from": a2}))
    # Check that mint to ZERO_ADDRESS failed
    w3_assert_tx_failed(lambda: c.mint(ZERO_ADDRESS, 1, transact={"from": a1}))
    w3_assert_tx_failed(lambda: c.mint(ZERO_ADDRESS, 1, transact={"from": minter}))


def test_totalSupply(c, w3, w3_assert_tx_failed):
...
def test_transfer(c, w3, w3_assert_tx_failed):
    minter, a1, a2 = w3.eth.accounts[0:3]
    w3_assert_tx_failed(lambda: c.burn(1, transact={"from": a2}))
    c.mint(a1, 2, transact={"from": minter})
    c.burn(1, transact={"from": a1})
    ...
    w3_assert_tx_failed(lambda: c.transfer(a1, 1, transact={"from": a2}))
    # Ensure 0-transfer always succeeds
    c.transfer(a1, 0, transact={"from": a2})


def test_maxInts(c, w3, w3_assert_tx_failed):
    minter, a1, a2 = w3.eth.accounts[0:3]
    c.mint(a1, MAX_UINT256, transact={"from": minter})
    assert c.balanceOf(a1) == MAX_UINT256
    w3_assert_tx_failed(lambda: c.mint(a1, 1, transact={"from": a1}))
    w3_assert_tx_failed(lambda: c.mint(a1, MAX_UINT256, transact={"from": a1}))
    # Check that totalSupply cannot overflow, even when mint to other account
    w3_assert_tx_failed(lambda: c.mint(a2, 1, transact={"from": minter}))
...
def test_transferFrom_and_Allowance(c, w3, w3_assert_tx_failed):
    ...
    w3_assert_tx_failed(lambda: c.transferFrom(a1, a3, 1, transact={"from": a2}))
    ...
    w3_assert_tx_failed(lambda: c.transferFrom(a1, a3, 1, transact={"from": a3}))
    ...
    w3_assert_tx_failed(lambda: c.transferFrom(a2, a3, 1, transact={"from": a1}))
...
def test_burnFrom_and_Allowance(c, w3, w3_assert_tx_failed):
    minter, a1, a2, a3 = w3.eth.accounts[0:4]
    w3_assert_tx_failed(lambda: c.burn(1, transact={"from": a2}))
    c.mint(a1, 1, transact={"from": minter})
    c.mint(a2, 1, transact={"from": minter})
    c.burn(1, transact={"from": a1})
    # This should fail; no allowance or balance (0 always succeeds)
    w3_assert_tx_failed(lambda: c.burnFrom(a1, 1, transact={"from": a2}))
    ...
    assert c.allowance(a2, a1) == 0
    # transferFrom should succeed when allowed, fail with wrong sender
    w3_assert_tx_failed(lambda: c.burnFrom(a2, 1, transact={"from": a3}))
    assert c.balanceOf(a2) == 1
    c.approve(a1, 1, transact={"from": a2})
    ...
    c.approve(a1, 1, transact={"from": a2})
    assert c.allowance(a2, a1) == 1
    w3_assert_tx_failed(lambda: c.burnFrom(a2, 1, transact={"from": a1}))
    # 0-approve should not change balance or allow transferFrom to change balance
    c.mint(a2, 1, transact={"from": minter})
    assert c.allowance(a2, a1) == 1
    c.approve(a1, 0, transact={"from": a2})
    ...
    w3_assert_tx_failed(lambda: c.burnFrom(ZERO_ADDRESS, 0, transact={"from": a1}))
...
def test_bad_transfer(c_bad, w3, w3_assert_tx_failed):
...
def test_bad_burn(c_bad, w3, w3_assert_tx_failed):
    ...
    w3_assert_tx_failed(lambda: c_bad.burn(3, transact={"from": a1}))


def test_bad_transferFrom(c_bad, w3, w3_assert_tx_failed):
    ...
    w3_assert_tx_failed(lambda: c_bad.transferFrom(a2, a1, 1, transact={"from": a1}))

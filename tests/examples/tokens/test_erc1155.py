...
def erc1155(get_contract, w3, assert_tx_failed):
    ...
    assert c.balanceOf(1, a1) == 1
    assert c.balanceOf(2, a1) == 1
    assert c.balanceOf(3, a1) == 1
    ...
    assert c.balanceOf(21, a1) == 1
    assert c.balanceOf(22, a1) == 1
    assert c.balanceOf(23, a1) == 1
    assert c.balanceOf(24, a1) == 1
...
def test_pause(erc1155,w3, assert_tx_failed):
    ...
    assert_tx_failed(lambda: erc1155.safeBatchTransferFrom(a1, a2, [21,22,23], [1,1,1], '', transact={"from": a1}))
...
def test_safeTransferFrom_balanceOf_single(erc1155, w3, assert_tx_failed):
    ...
    assert_tx_failed(lambda: erc1155.safeTransferFrom(a1, ZERO_ADDRESS, 24, 1, '', transact={"from": a1}))
    ...
    assert erc1155.balanceOf(21, a2) == 1
    
    # try to transfer item again
    assert_tx_failed(lambda: erc1155.safeTransferFrom(a1, a2, 21, 1, '', transact={"from": a1}))
    assert erc1155.balanceOf(21, a1) == 0
...
def test_mintBatch_balanceOf(erc1155, w3, assert_tx_failed):  # test_mint_batch
    ...
    for i in range (1,10):
        assert erc1155.balanceOf(i, a1) == 1
...
def test_safeBatchTransferFrom_balanceOf_batch(erc1155, w3, assert_tx_failed):  # test_mint_batch
    ...
    assert erc1155.balanceOf(21, a1) == 1
    assert erc1155.balanceOf(22, a1) == 1
    assert erc1155.balanceOf(23, a1) == 1
    assert erc1155.balanceOf(23, a1) == 1

    # try to transfer item from non item owner account
    assert_tx_failed(lambda: erc1155.safeBatchTransferFrom(a1, a2, [21,22,23], [1,1,1], '', transact={"from": a2}))

    # try to transfer item to zero address
    assert_tx_failed(lambda: erc1155.safeBatchTransferFrom(a1, ZERO_ADDRESS, [21,22,23], [1,1,1], '', transact={"from": a1}))

    # try to transfer item to self
    assert_tx_failed(lambda: erc1155.safeBatchTransferFrom(a1, a1, [21,22,23], [1,1,1], '', transact={"from": a1}))
    
    # try to transfer more items than we own
    assert_tx_failed(lambda: erc1155.safeBatchTransferFrom(a1, a2, [21,22,23], [1,125,1], '', transact={"from": a1}))

    # mismatched item and amounts
    assert_tx_failed(lambda: erc1155.safeBatchTransferFrom(a1, a2, [21,22,23], [1,1], '', transact={"from": a1}))

    # try to transfer nonexisting item
    assert_tx_failed(lambda: erc1155.safeBatchTransferFrom(a1, a2, [21,22,500], [1,1,1], '', transact={"from": a1}))
...
def test_mint_one_burn_one(erc1155, w3, assert_tx_failed):
    ...
    assert erc1155.balanceOf(25, owner) == 1
    assert erc1155.balanceOf(25, owner) == 1
    ...
    assert erc1155.balanceOf(25, owner) == 0

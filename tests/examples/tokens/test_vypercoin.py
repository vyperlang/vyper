...
def test_transfer_event(w3, c, get_logs):
    ...
    logs = get_logs(c.transfer(a1, 1, transact={}), 'Transfer')
    ...
    logs = get_logs(c.transferFrom(a0, a2, 4, transact={'from': a1}), 'Transfer') # transfer to a2, as a1, from a0's funds.
...
def test_approval_event(w3, c, get_logs):
    ...
    logs = get_logs(c.approve(a1, 10, transact={}), 'Approval') # approve 10 token transfers to a1.

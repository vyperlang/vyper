...
def test_balanceOf(c, w3, assert_w3_tx_failed):
    someone = w3.eth.accounts[1]
    assert c.balanceOf(someone) == 3
    assert_w3_tx_failed(lambda: c.balanceOf(ZERO_ADDRESS))


def test_ownerOf(c, w3, assert_w3_tx_failed):
    someone = w3.eth.accounts[1]
    assert c.ownerOf(SOMEONE_TOKEN_IDS[0]) == someone
    assert_w3_tx_failed(lambda: c.ownerOf(INVALID_TOKEN_ID))
...
def test_transferFrom_by_owner(c, w3, assert_w3_tx_failed, get_logs):
    someone, operator = w3.eth.accounts[1:3]
...
def test_safeTransferFrom_by_owner(c, w3, assert_w3_tx_failed, get_logs):
    someone, operator = w3.eth.accounts[1:3]
...
def test_safeTransferFrom_to_contract(c, w3, assert_w3_tx_failed, get_logs, w3_get_contract):
    someone = w3.eth.accounts[1]
...
def test_approve(c, w3, assert_w3_tx_failed, get_logs):
    someone, operator = w3.eth.accounts[1:3]
    ...
    assert_w3_tx_failed(lambda: c.approve(operator, INVALID_TOKEN_ID, transact={"from": someone}))
...
def test_setApprovalForAll(c, w3, assert_w3_tx_failed, get_logs):
    someone, operator = w3.eth.accounts[1:3]
    approved = True

    # setApprovalForAll myself
    assert_w3_tx_failed(lambda: c.setApprovalForAll(someone, approved, transact={"from": someone}))
...
def test_mint(c, w3, assert_w3_tx_failed, get_logs):
...
def test_burn(c, w3, assert_w3_tx_failed, get_logs):
    someone, operator = w3.eth.accounts[1:3]

    # burn token without ownership
    assert_w3_tx_failed(lambda: c.burn(SOMEONE_TOKEN_IDS[0], transact={"from": operator}))
    ...
    assert args.tokenId == SOMEONE_TOKEN_IDS[0]
    assert_w3_tx_failed(lambda: c.ownerOf(SOMEONE_TOKEN_IDS[0]))
    assert c.balanceOf(someone) == 2

...
def c(w3, w3_get_contract):
    with open("examples/stock/company.vy") as f:
        ...
        contract = w3_get_contract(contract_code, *[w3.eth.accounts[0], 1000, 10 ** 6])
    return contract


def test_overbuy(w3, c, assert_w3_tx_failed):
    ...
    one_stock = c.price()
    assert_w3_tx_failed(lambda: c.buyStock(transact={"from": a1, "value": one_stock}))
    assert_w3_tx_failed(lambda: c.buyStock(transact={"from": a2, "value": one_stock}))


def test_sell_without_stock(w3, c, assert_w3_tx_failed):
    a1, a2 = w3.eth.accounts[1:3]
    # If you don't have any stock, you can't sell
    assert_w3_tx_failed(lambda: c.sellStock(1, transact={"from": a1}))
    assert_w3_tx_failed(lambda: c.sellStock(1, transact={"from": a2}))
...
def test_oversell(w3, c, assert_w3_tx_failed):
    a0, a1, a2 = w3.eth.accounts[:3]
    # You can't sell more than you own
    test_shares = int(c.totalShares())
    test_value = int(test_shares * c.price())
    c.buyStock(transact={"from": a1, "value": test_value})
    assert_w3_tx_failed(lambda: c.sellStock(test_shares + 1, transact={"from": a1}))


def test_transfer(w3, c, assert_w3_tx_failed):
    # If you don't have any stock, you can't transfer
    a1, a2 = w3.eth.accounts[1:3]
    assert_w3_tx_failed(lambda: c.transferStock(a2, 1, transact={"from": a1}))
    assert_w3_tx_failed(lambda: c.transferStock(a1, 1, transact={"from": a2}))
...
def test_paybill(w3, c, assert_w3_tx_failed):
    a0, a1, a2, a3 = w3.eth.accounts[:4]
    # Only the company can authorize payments
    assert_w3_tx_failed(lambda: c.payBill(a2, 1, transact={"from": a1}))
    # A company can only pay someone if it has the money
    assert_w3_tx_failed(lambda: c.payBill(a2, 1, transact={"from": a0}))
    ...
    assert_w3_tx_failed(lambda: c.payBill(a3, 1, transact={"from": a0}))

...
def test_overbuy(w3, c, w3_assert_tx_failed):
    ...
    one_stock = c.price()
    w3_assert_tx_failed(lambda: c.buyStock(transact={"from": a1, "value": one_stock}))
    w3_assert_tx_failed(lambda: c.buyStock(transact={"from": a2, "value": one_stock}))


def test_sell_without_stock(w3, c, w3_assert_tx_failed):
    a1, a2 = w3.eth.accounts[1:3]
    # If you don't have any stock, you can't sell
    w3_assert_tx_failed(lambda: c.sellStock(1, transact={"from": a1}))
    w3_assert_tx_failed(lambda: c.sellStock(1, transact={"from": a2}))
...
def test_oversell(w3, c, w3_assert_tx_failed):
    a0, a1, a2 = w3.eth.accounts[:3]
    # You can't sell more than you own
    test_shares = int(c.totalShares())
    test_value = int(test_shares * c.price())
    c.buyStock(transact={"from": a1, "value": test_value})
    w3_assert_tx_failed(lambda: c.sellStock(test_shares + 1, transact={"from": a1}))


def test_transfer(w3, c, w3_assert_tx_failed):
    # If you don't have any stock, you can't transfer
    a1, a2 = w3.eth.accounts[1:3]
    w3_assert_tx_failed(lambda: c.transferStock(a2, 1, transact={"from": a1}))
    w3_assert_tx_failed(lambda: c.transferStock(a1, 1, transact={"from": a2}))
...
def test_paybill(w3, c, w3_assert_tx_failed):
    a0, a1, a2, a3 = w3.eth.accounts[:4]
    # Only the company can authorize payments
    w3_assert_tx_failed(lambda: c.payBill(a2, 1, transact={"from": a1}))
    # A company can only pay someone if it has the money
    w3_assert_tx_failed(lambda: c.payBill(a2, 1, transact={"from": a0}))
    ...
    w3_assert_tx_failed(lambda: c.payBill(a3, 1, transact={"from": a0}))

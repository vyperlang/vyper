...
def test_bool_kwarg_uses_reduced_value():
    ...
    assert (
        get_bool_kwarg(call_node, "flag", True, allowed_kwarg_names=("flag",)) is False
    )
...
def test_literal_kwarg_uses_reduced_value():
    ...
    assert (
        get_literal_kwarg(
            ...
            "revert_on_failure",
            True,
            allowed_kwarg_names=("revert_on_failure",),
        )
        is False
    )
...
def test_literal_kwarg_rejects_unreduced_value():
    ...
    with pytest.raises(
        CompilerPanic, match="unfoldable literal kwarg: revert_on_failure"
    ):

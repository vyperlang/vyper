...
from tests.venom_utils import PrePostChecker, parse_from_basic_block, assert_ctx_eq
...
class VolatilePrePostChecker(PrePostChecker):
    ...
    def __call__(self, pre: str, post: str, hevm: bool | None = None) -> list[IRPass]:
        from vyper.venom.basicblock import MemoryLocation, IROperand
...
def test_volatile_location_store():
    ...
    pre = """
        _global:
            %val = 42
            mstore 0xFFFF0000, %val  ; This store should not be eliminated because the location will be marked as volatile
            stop
    """

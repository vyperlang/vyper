...
def test_allocate_overlapping_reserved_intervals():
    ...
    imm_inst = IRInstruction("alloca", [IRLiteral(480)], outputs=[IRVariable("%imm")])
    tmp_inst = IRInstruction("alloca", [IRLiteral(96)], outputs=[IRVariable("%tmp")])
    buf_inst = IRInstruction("alloca", [IRLiteral(96)], outputs=[IRVariable("%buf")])

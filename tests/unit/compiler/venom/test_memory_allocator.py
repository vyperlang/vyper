...
def test_allocate_overlapping_reserved_intervals():
    ...
    imm_inst = IRInstruction(
        "alloca", [IRLiteral(480), IRLiteral(1)], outputs=[IRVariable("%imm")]
    )
    tmp_inst = IRInstruction(
        "alloca", [IRLiteral(96), IRLiteral(2)], outputs=[IRVariable("%tmp")]
    )
    buf_inst = IRInstruction(
        "alloca", [IRLiteral(96), IRLiteral(3)], outputs=[IRVariable("%buf")]
    )

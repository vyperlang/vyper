...
a: uint256
    ...
    with pytest.raises(StorageLayoutException) as exc_info:
        compile_code(
            code,
            output_formats=["layout"],
            storage_layout_override=json_input(storage_layout_override),
    ...
    assert exc_info.value.hint == (
        ...
        "for example one generated by `vyper -f layout`?"
    )
...
a: uint256
    ...
    assert exc_info.value.message == ("invalid storage slot for a: expected a dict, got a int: 5")
...
def foo():
    ...
    assert exc_info.value.message == "no storage slot for $.nonreentrant_key"
